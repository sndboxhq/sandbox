...
sndbox currently implements **60 built-in nodes** across triggers, logic, data, network, browser, communication, and system automation. These pages are generated from `src/catalogue.ts`; names, type IDs, defaults, typed ports, side-effect flags, and supported placements are not hand-copied.
...
<Card title="Validate Schema" icon="git-branch" href="/nodes/validate-schema">
  Route data by required fields and sndbox value types.
</Card>
...
<Card title="Map Fields" icon="braces" href="/nodes/map-fields">
  Project, rename, and default object fields without code.
</Card>
<Card title="Text Template" icon="braces" href="/nodes/text-template">
  Render text with safe workflow expressions.
</Card>
<Card title="Hash Data" icon="braces" href="/nodes/hash-data">
  Create a deterministic SHA-256 digest of JSON data.
</Card>
...
<Card title="Custom Function" icon="braces" href="/nodes/custom-function">
  A verified local ƒx node with an editable contract.
</Card>
