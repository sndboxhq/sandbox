...
const nodeFieldDescriptions: Record<string, string> = {
  ...
  "map_fields.mappings":"Ordered source-to-target field mappings with required and default behavior.","map_fields.preserveUnmapped":"Keep source fields that are not explicitly mapped.",
  "validate_schema.rules":"Ordered field-path, type, and required-value checks.","validate_schema.allowAdditionalFields":"Permit fields that are not named by a validation rule.",
  "text_template.values":"Optional explicit data made available to the template.","text_template.template":"Text rendered with the safe workflow expression resolver.",
  "hash_data.encoding":"Digest output encoding. The current node emits lowercase hexadecimal SHA-256.",
...
const nodeGuides: Record<string, NodeGuide> = {
  ...
  map_fields:{useCases:["Project or rename object fields without writing code.","Apply explicit defaults while normalizing records from another system."],example:{title:"Normalize a customer record",description:"Rename first_name to name, require the source field, and omit unrelated input properties.",configuration:{mappings:[{source:"first_name",target:"name",required:true,default:null}],preserveUnmapped:false},flow:["Connect an object-producing node.","Add one mapping for each output field.","Test missing required and optional fields before using the result."]},behavior:["Mappings are applied in configured order and produce an explicit object shape.","Missing required fields fail; optional fields can use their configured default."],commonIssues:["Source and target values are object paths, not template expressions.","Enable preserveUnmapped only when downstream consumers should receive the remaining source data."],related:[{label:"Variables and data mapping",href:"/workflows/variables-and-data"},{label:"JSON data",href:"/files-and-data/json"}]},
  validate_schema:{useCases:["Check required fields and value types before processing external data.","Route valid and invalid values through visible branches."],example:{title:"Validate an imported record",description:"Require a numeric id while allowing other fields to pass through.",configuration:{rules:[{path:"id",type:"number",required:true}],allowAdditionalFields:true},flow:["Connect the value that crosses the trust boundary.","Describe each required path and exact sndbox type.","Connect both Valid and Invalid branches deliberately."]},behavior:["Validation reports a boolean, the original value, and bounded field errors.","Invalid data follows the Invalid branch instead of relying on coercion."],commonIssues:["The string \"1\" is not the number 1.","Disable additional fields only when the complete accepted shape is represented by rules."],related:[{label:"Data types",href:"/reference/data-types"},{label:"Branches and conditions",href:"/workflows/branches-and-conditions"}]},
  text_template:{useCases:["Build a readable message from structured workflow data.","Create small text bodies without granting a code runtime."],example:{title:"Create a greeting",description:"Render a name from the connected input object.",configuration:{values:null,template:"Hello {{input.name}}"},flow:["Connect or configure the template data.","Insert safe workflow expressions in the text.","Test missing values and multiline output before sending it externally."]},behavior:["Expressions use the same restricted resolver as other mapped workflow fields.","Rendering produces text only and has no filesystem, network, or process access."],commonIssues:["Use the input root for connected data and verify the field path in test output.","Treat rendered secrets as sensitive because later logs or messages can expose them."],related:[{label:"Expressions",href:"/workflows/expressions"},{label:"Variables and data mapping",href:"/workflows/variables-and-data"}]},
  hash_data:{useCases:["Create a deterministic SHA-256 fingerprint for JSON-compatible data.","Compare content without carrying the complete value into a later decision."],example:{title:"Fingerprint a normalized record",description:"Hash one stable object after field mapping.",configuration:{encoding:"hex"},flow:["Normalize volatile or unordered source fields first.","Connect the value to Hash Data.","Store or compare the digest while retaining the original only when needed."]},behavior:["Objects are canonicalized before hashing so property insertion order does not change the digest.","The node returns a lowercase hexadecimal digest and the canonical byte count."],commonIssues:["Semantically different types produce different digests; string 1 and number 1 are not interchangeable.","A hash is an integrity fingerprint, not encryption or secret storage."],related:[{label:"JSON data",href:"/files-and-data/json"},{label:"Data types",href:"/reference/data-types"}]},
  ...
  custom_function: {
    useCases: ["Turn a focused JavaScript or Python transformation into a named local workflow node.", "Define explicit inputs, outputs, and branches when a built-in node does not fit."],
    example: {
      title: "Normalize an internal record",
      description: "Define a required object input, return a typed normalized output, and verify the behavior against representative fixtures.",
      configuration: {},
      flow: ["Open the full-page custom function editor and choose JavaScript or Python.", "Define the contract before writing the implementation.", "Add fixtures, run verification, and review coverage before connecting the node."],
    },
    behavior: ["Custom functions run only on the local desktop in a restricted runtime with no ambient filesystem, network, environment, credential, browser, or plugin access.", "A SHA-256 receipt binds successful verification to the exact function contract, source, and fixtures."],
    commonIssues: ["Imports, duplication, and material edits invalidate the verification receipt.", "Keep the contract narrow and use ordinary permissioned nodes for external side effects."],
    related: [{ label: "Code and Web Builder", href: "/files-and-data/code-and-web-builder" }, { label: "Permissions", href: "/workflows/permissions" }],
  },
  web_builder: {
